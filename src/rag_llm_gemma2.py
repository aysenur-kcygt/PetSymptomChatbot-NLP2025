# src/rag_llm_gemma2.py
# Strict triage-style RAG generation for Ollama (Gemma 2 default)
# Goals:
...
def _extract_species_from_prompt(text: str) -> Optional[str]:
    ...
    Works best when query includes the Pet profile block too.
    """
    t = (text or "").lower()
...
def _ensure_disclaimer(text: str) -> str:
    ...
    # Ensure it exists exactly once at the end
    t = re.sub(r"(?is)\n\s*This is not medical advice\.\s*$", "", t).strip()
...
def _build_prompts(query: str, context_text: str) -> Tuple[str, str]:
    ...
    # Shorter system prompt reduces “templated echoing”
    system_prompt = (
        ...
        "Do not list diseases or conditions unless the user explicitly asks what it could be."
    )
    ...
    if missing:
        extra_rules.append(
            "- Key details are missing. Use the 'Questions to clarify' section heavily and keep other sections to 1–2 bullets max."
        )
...
def generate_answer(
    ...
    """
    Generate a grounded triage-style response using Ollama (Gemma 2).
    Returns: (text, meta)
    """
